...
grouped = GROUP filtered BY getBlogName(blogs);
all_hits = FILTER grouped by hits(filtered.results);
num_hits = FOREACH all_hits GENERATE group, COUNT(filtered.results) as hit_count;
all_errors = FILTER grouped by errors(filtered.results);
num_errors = FOREACH all_errors GENERATE group, COUNT(filtered.results) as err_count;
